...
foreach ($file in $allSourceFiles) {
  ...
  if ($imageExtensions -contains "*$originalExtension") {
    ...
    } elseif ($originalExtension -eq ".jpg") {
      # It's a JPG, but needs renaming (case or unique name)
      $pendingActions += @{
        Type         = "ImageRename"
        OriginalPath = $file.FullName
        NewPath      = $newPath
  ...
  # Handle Video Files
  elseif ($videoExtensions -contains "*$originalExtension") {
    ...
    } elseif ($originalExtension -eq ".mp4") {
      # It's an MP4, but needs renaming
      $pendingActions += @{
        Type         = "VideoRename"
        OriginalPath = $file.FullName
        NewPath      = $newPath
...
if ($PSCmdlet.ShouldProcess("process media files recursively in '$SourcePath'", "Perform Conversion/Rename")) {
  ...
  foreach ($action in $pendingActions) {
    ...
    if ($conversionSuccessful -and ($action.OriginalPath.ToLower() -ne $action.NewPath.ToLower())) {
      Read-OriginalFile -OriginalFilePath $action.OriginalPath -MoveOriginals:$MoveOriginals -SourceBaseDir $SourcePath
